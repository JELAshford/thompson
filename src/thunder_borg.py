...
class ThunderBorg:
    ...
    def check_chip(self):
        ...
        i2cRecv = self.try_read(COMMAND_GET_ID, fail_message=f"Missing ThunderBorg at {self.i2cAddress}")
        ...
        if device_responded and device_is_thunderborg:
            self.foundChip = True
            print(f"Found ThunderBorg at {self.i2cAddress} loaded on bus {self.busNumber}")
        elif device_responded:
            print(f"Found a device at {self.i2cAddress}, but it is not a ThunderBorg (ID {i2cRecv[1]} instead of {I2C_ID_THUNDERBORG})")
        else: 
            print(f"Missing ThunderBorg at {self.i2cAddress}")
            print('Are you sure your ThunderBorg is properly attached, the correct address is used, and the I2C drivers are running?')
    ...
    def get_motor_power(self, motor: int = 1):
        ...
        i2cRecv = self.try_read(commands[motor], fail_message=f"Failed reading motor {motor} drive level!")
    ...
    def get_led(self, led: int = 1):
        ...
        i2cRecv = self.try_read(commands[led], fail_message=f"Failed reading ThunderBorg LED {led} colour!")
    ...
    def set_led(self, led: int = 1, rgb = (0., 0., 0.)):
        ...
        self.try_write(commands[led], [levelR, levelG, levelB], f'Failed sending colour for LED  {led}!')
    ...
    def set_comms_failsafe(self, state: bool):
        ...
        self.try_write(COMMAND_SET_FAILSAFE, [level], 'Failed sending communications failsafe state!')


    def get_comms_failsafe(self):
        """Read the current system state of the communications failsafe""" 
        i2cRecv = self.try_read(COMMAND_GET_FAILSAFE, fail_message=f"Failed reading communications failsafe state!")
    ...
    def get_drive_fault(self, motor: int = 1) -> bool:
        ...
        i2cRecv = self.try_read(commands[motor], fail_message=f"Failed reading the drive fault state for motor #{motor}!")
        return i2cRecv[1] != COMMAND_VALUE_OFF


    def get_battery_reading(self):
        """Reads battery level as a voltage based on the 3.3 V rail reference.""" 
        i2cRecv = self.try_read(COMMAND_GET_BATT_VOLT, fail_message=f"Failed reading battery level!")
